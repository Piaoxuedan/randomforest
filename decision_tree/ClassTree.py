...
class ClassificationTree:
    ...
    def __entropy(self, labels):
        if len(labels) == 0:
            return 0.
        classes, props = self.__classshares(labels)  #props 是一个2*1的矩阵 每一行代表每一类的比例
        entropy = -np.dot( props.T, np.log2(props+0.00001) )  #求的是基本熵 info（D）
        return entropy[0][0]
    ...
    def __gini(self, labels):
        if len(labels) == 0:
            return 0.
        classes, props = self.__classshares(labels)  #目前调用的是gini
        gini = 1 - np.dot(props.T, props)  #训练元组集D的不纯度
        return gini[0][0]
    ...
    def __bestsplit(self, feature, labels):
        ...
        bestentropy = np.inf  #bestentropy:inf type:float
        bestsplit = 0
        #check if the number of values isn't too large
        if len(values) > 10:
            minv = values.min()
            maxv = values.max()
            values = np.arange(minv, maxv, (maxv-minv)/10.) #算左不算右
        for v in values:
            leftmask = feature <= v
            rightmask = feature > v
